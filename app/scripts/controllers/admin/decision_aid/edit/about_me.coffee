...
class DecisionAidEditAboutMeCtrl
  ...
  constructor: (@$scope, @$state, @$stateParams, @Sortable, @DecisionAid, @Question, @Confirm, @AdminTabHelper, @_, @$uibModal, @ErrorHandler, @QuestionPage, @$q) ->
    ...
    questions_p = @Question.query({decision_aid_id: @$stateParams.decisionAidId, question_type: "demographic", include_hidden: true}).$promise
    question_pages_p = @QuestionPage.query({decision_aid_id: @$stateParams.decisionAidId, section: "about", include_questions: true}).$promise
    ...
    if question
      ...
      if partFrom isnt partTo
        if questionPage
          ...
          @moveQuestionToPage(question, questionPage, partTo, partFrom) if questionPage 
      else if partFrom is partTo and question.question_order isnt indexTo + 1
        question.question_order = indexTo + 1
  ...
  moveQuestionToPage: (question, page, toArr, fromArr) ->
  ...
  preview: (da) ->
    @$uibModal.open(
      templateUrl: "/views/admin/decision_aid/preview/about_me.html"
      controller: "AboutMePreviewCtrl"
      size: 'lg'
      resolve:
        options: () =>
          decisionAid: @decisionAid
...
module.controller 'DecisionAidEditAboutMeCtrl', DecisionAidEditAboutMeCtrl
